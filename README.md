...
- 支持 `sourceKey` / `originalKey` 双字段模式：当贴入内容的调与真实原调不同，页面默认应直接展示 `originalKey`，而不是先展示输入时的 `sourceKey`
- 对“和弦行 + 歌词括号定位”文本：和弦只应落在括号明确标出的点上，不得默认在句首额外补和弦
...
- 页面整体偏紧凑，目标是减少翻页
- 转调工具放在标题区附近，采用横向工具条，不使用右侧悬浮方案
- 工具条内显示当前 key 与原调
- 纯音乐段落也必须参与 transpose，不允许只正文和弦转调
- 歌曲头部信息收敛为单行 meta，避免重复展示与无效装饰；原调不再重复出现在语言 / 风格 / 歌手那一行
- 对《黑洞裡》这类长行曲谱：桌面端优先保留对位，移动端允许使用“阅读优先版”渲染，减少超长单行造成的阅读阻碍
```
