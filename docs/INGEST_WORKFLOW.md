...
6. 若脚本自动添加结构标签，统一输出为 `***verse***` / `***chorus***` 这类格式；且只有在**高置信**命中明确结构词时才允许自动加，不能为了“看起来完整”而猜 section
7. 和弦 token 识别需兼容常见扩展写法，至少覆盖 `C11sus` / `C11sus4` / `C7sus4` / `Csusb9` 这类 PDF 中高频出现的后缀形式；前端与 PDF 脚本应尽量共用同一套语法口径
8. PDF 中普通括号文本（如 `(love)` / `(love song)` / `(oh-oh, ooh-ooh)` / `(To Bridge)` / `(Bb key)`）默认不应自动按 chord 处理；只有高置信和弦 token 才能进入和弦解析
9. chord 注入时，不应只按“最近字符”粗暴插入；应优先锚到词起点 / 空格后首字，减少把和弦插进英文单词中间的情况
10. 对 `1.F 2. Am (To Bridge)`、`(Bb key)`、ending 提示等混合行，需要允许“以和弦为主、说明为辅”的分类，不要把整行误判成 lyric line
11. chord-only / 无歌词段若带有 PDF 噪音（如 `F. - Bb. - F. - Bbm. Csusb9`），应在脚本中清洗点号、连字符等版面符号，输出为可直接渲染/转调的标准和弦行
12. 生成的 md 默认视为“坐标驱动草案”，仍需本地验收
13. 若目标是尽量接近 PDF 页面视觉效果，后续可继续增强渲染层，而不只停留在普通文本流
